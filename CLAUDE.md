...
## Текущее состояние (Этап 7)
- Деплой на Render (free tier): render.yaml в корне (build: pip install -r
  requirements.txt; start: uvicorn main:app --host 0.0.0.0 --port $PORT;
  ANTHROPIC_API_KEY как sync:false — секрет вводится руками в Render
  Dashboard → Environment, не в репозитории).
- Живой URL: https://document-auditor.onrender.com — проверено реальным
  curl-запросом с sample.txt → 200 + валидный AuditReport (8 найденных
  ошибок, suggested_fix заполнен где применимо).
- README.md написан с нуля (раньше не было): живая ссылка вверху,
  curl-пример с публичным URL, метрики Этапа 6 (P=0.769/R=0.833/F1=0.8)
  со ссылкой на eval/README.md, секция "Run locally".
- docs/demo_script.md — сценарий 60-секундного демо (5 битов: контекст →
  показать документ → curl-запрос на живой URL → разбор JSON-ответа →
  закрывающий кадр с репо+метриками); запись видео — вне скоупа этой
  сессии, делает пользователь сам.
- Репозиторий публичный (был и до этого этапа), .env не трекался никогда,
  секретов в истории нет — проверено `git ls-files`/`git log` перед коммитом.

## Что нужно доделать в Этапе 7
Этап 7 закрыт полностью. Видео физически не записано (вне скоупа сессии
по договорённости с пользователем) — сценарий готов в docs/demo_script.md.
